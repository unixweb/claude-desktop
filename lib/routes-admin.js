...
const { disableTotp } = require('./totp');
const {
  listUsersWithStats,
  getTotals,
  setAdmin,
...
async function requireAdmin(req, res, next) {
  ...
  try {
    const fresh = await findUserByEmail(req.session.user.email);
    if (!fresh.totp_enabled_at) {
      req.flash('error', 'Admins müssen 2FA aktivieren, bevor sie den Admin-Bereich nutzen können.');
...
router.post('/users/:id/reset-2fa', async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    if (id === req.session.user.id) {
      req.flash('error', 'Eigene 2FA bitte unter Account → Sicherheit verwalten.');
      return res.redirect('/admin');
    }
    await disableTotp(id);
    req.flash('success', '2FA für diesen User zurückgesetzt.');
    res.redirect('/admin');
  } catch (err) {
    next(err);
  }
